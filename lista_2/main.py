        ...
        for e in epsilon:
            ...
            for dataset_id, dataset_name in enumerate(datasets):
                ...
                for version in versions.keys():
                    results_dir_cur = os.path.join(RESULTS_DIR, f"lr={lr}_w={window}_e={e}")
                    results_dir = os.path.join(results_dir_cur, versions[version])
                    ...
                    for n_neighbors in all_n_neighbors:
                        ...
                        for n_codebooks in all_n_codebooks:
                            ...
                            # Getting cookbooks vectors -> LVQ1, LVQ1 + LVQ2.1 or LVQ1 + LVQ3
                                ...
                                if version != "1":
                                    data = train_codebooks(data_LVQ1, n_codebooks, lr, n_epochs, version=version, w=window, epsilon=e)
                                else:
                                    data = data_LVQ1
                            else:
                                data = raw_data
